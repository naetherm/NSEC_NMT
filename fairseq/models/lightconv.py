...
class LightConvModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-linear-se', action='store_true')
    ...
    def build_model(cls, args, task):
        ...
        encoder = LightConvEncoder(args, src_dict, encoder_embed_tokens, use_linear_se=args.use_linear_se)
        decoder = LightConvDecoder(args, tgt_dict, decoder_embed_tokens, use_linear_se=args.use_linear_se)
...
class LightConvDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, final_norm=True, use_linear_se=False):
        super().__init__(dictionary)
        self.dropout = args.dropout
        ...
        if use_linear_se:
            self.project_in_dim = LinearSE(input_embed_dim, embed_dim, bias=False) if embed_dim != input_embed_dim else None
        else:
        ...
        self.layers = nn.ModuleList([])
        self.layers.extend([
            LightConvDecoderLayer(args, no_encoder_attn, kernel_size=args.decoder_kernel_size_list[i], use_linear_se=use_linear_se)
        ...
        if use_linear_se:
            self.project_out_dim = LinearSE(embed_dim, output_embed_dim, bias=False) \
...
class LightConvEncoderLayer(nn.Module):
    ...
    def __init__(self, args, kernel_size=0, use_linear_se=False):
        super().__init__()
        ...
        if use_linear_se:
            self.fc1 = LinearSE(self.embed_dim, args.encoder_ffn_embed_dim)
            self.fc2 = LinearSE(args.encoder_ffn_embed_dim, self.embed_dim)
        else:
...
def LinearSE(in_features, out_features, bias=True):
    """Weight-normalized Linear layer (input: N x T x C)"""
    m = SqueezeExcitationLayer(in_features, out_features, bias=bias)
...
def base_architecture(args):
    ...
    args.use_linear_se = getattr(args, 'use_linear_se', True)
