...
@register_model('mslightconv')
class MSLightConvModel(FairseqEncoderDecoderModel):
    """ TODO(naetherm): Multi-Source implementation of lightconv, so that we can use character and word based embeddings
    ...
    def add_args(parser):
        ...
        ## BEGIN
        # These are two additional sources for loading pretrained word embeddings. We will usually use GloVe here
        parser.add_argument('--encoder-embed-word-path', type=str, metavar='STR',
                            help='path to pre-trained encoder word embedding')
        # The dimensionality of the use word embeddings
        parser.add_argument('--encoder-embed-word-dim', type=int, metavar='N',
                            help='encoder word embedding dimension')
        # 
        ## END
    ...
    def build_model(cls, args, task):
        ...
        def build_encoder_embedding(dictionary, word_embed_dim: int, char_embed_dim: int,  glove_path:str = None):
            pass
        ...
        if args.encoder_embed_word_path:
            # Build the word embeddings
            coder_embed_words = build_embedding(
                42, 
                300,
                args.encoder_embed_word_path
            )
        else:
            coder_embed_words = None

        encoder = MSLightConvEncoder(args, src_dict, encoder_embed_tokens, coder_embed_words)
        decoder = MSLightConvDecoder(args, tgt_dict, decoder_embed_tokens, coder_embed_words)
        return MSLightConvModel(encoder, decoder)


class MSLightConvEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_words):
...
class MSLightConvDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, embed_words, no_encoder_attn=False, final_norm=True):
        super().__init__(dictionary)
        self.dropout = args.dropout
...
@register_model_architecture('mslightconv', 'mslightconv')
def base_architecture(args):
    ...
    args.encoder_embed_word_path = getattr(args, 'encoder_embed_word_path', None)
    args.decoder_embed_word_path = getattr(args, 'decoder_embed_word_path', None)
...
@register_model_architecture('mslightconv', 'mslightconv_iwslt_de_en')
def mslightconv_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('mslightconv', 'mslightconv_wmt_en_de')
def mslightconv_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('mslightconv', 'mslightconv_wmt_en_de_big')
def mslightconv_wmt_en_de_big(args):
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
...
@register_model_architecture('mslightconv', 'mslightconv_wmt_en_fr_big')
def mslightconv_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    mslightconv_wmt_en_de_big(args)


@register_model_architecture('mslightconv', 'mslightconv_wmt_zh_en_big')
def mslightconv_wmt_zh_en_big(args):
