...
@BaseFairseqModel.register("fconv")
...
class FConvModel(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, args, task):
        ...
        return FConvModel(args=args, task=task)
...
def base_architecture(args):
    ...
    args.use_linear_se = getattr(args, 'use_linear_se', False)


@register_model_architecture('fconv', 'fconv_iwslt_de_en_light')
def fconv_iwslt_de_en_light(args):
...
@register_model_architecture('fconv', 'fconv_iwslt_de_en_7layer')
def fconv_iwslt_de_en_7layer(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
...
@register_model_architecture('fconv', 'fconv_iwslt_de_en_ks7')
def fconv_iwslt_de_en_ks7(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 256)
    args.encoder_layers = getattr(args, 'encoder_layers', '[(256, 7)] * 4')
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 256)
    args.decoder_layers = getattr(args, 'decoder_layers', '[(256, 7)] * 3')
    args.decoder_out_embed_dim = getattr(args, 'decoder_out_embed_dim', 256)
    base_architecture(args)
...
def fconv_wmt_en_de(args):
    convs = '[(768, 3)] * 6'  # first 9 layers have 512 units
    convs += ' + [(768, 3)] * 2'  # next 4 layers have 1024 units
    convs += ' + [(768, 1)] * 2'  # final 2 layers use 1x1 convolutions 2048 units
