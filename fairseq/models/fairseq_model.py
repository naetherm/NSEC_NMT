...
from fairseq.modules.seq2seq_decoders.fairseq_decoder import FairseqDecoder
from fairseq.modules.seq2seq_encoders.fairseq_encoder import FairseqEncoder


class BaseFairseqModel(nn.Module, Registrable):
...
class FairseqMultiEncoderSingleDecoderModel(BaseFairseqModel):
    """Base class for encoders-decoder models.

    Args:
        encoders (List[FairseqEncoder]): the encoders
        decoder (FairseqDecoder): the decoder
    """
    ...
    def __init__(self, encoders, decoder):
        ...
        for enc in self.encoders:
            assert isinstance(enc, FairseqEncoder)
        assert isinstance(self.decoder, FairseqDecoder)
    ...
    def forward(self, src_tokens, src_lengths, prev_output_tokens, **kwargs):
        ...
        for enc in self.encoders:
            encoder_out = enc(encoder_input, src_lengths=src_lengths, **kwargs)
    ...
    def max_positions(self):
        ...
        We can just take the first encoder because we ensure that all encoders will have the 
        same max positions.
        """
        return (self.encoders[0].max_positions(), self.decoder.max_positions())
