...
from fairseq.modules.dropout_linear import DropoutLinear
from fairseq.modules.snu import LinearSNU, RecurrentSNU
...
@register_model('rnnlightconv')
class RNNLightConvModel(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-dropout-linearity', action='store_true')
        parser.add_argument('--use-snu', action='store_true')
    ...
    def build_model(cls, args, task):
        ...
        return RNNLightConvModel(encoder, decoder)
...
class LightConvDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, final_norm=True):
        ...
        self.use_dropout_linearity = False
        self.use_snu = False
        ...
        if self.use_dropout_linearity:
            self.project_in_dim = DropoutLinear(input_embed_dim, embed_dim, bias=False) if embed_dim != input_embed_dim else None
        elif self.use_snu:
            self.project_in_dim = LinearSNU(input_embed_dim, embed_dim, bias=False) if embed_dim != input_embed_dim else None
        else:
        ...
        if self.use_dropout_linearity:
            self.project_out_dim = DropoutLinear(embed_dim, output_embed_dim, bias=False) if embed_dim != output_embed_dim and not args.tie_adaptive_weights else None
        elif self.use_snu:
            self.project_out_dim = LinearSNU(embed_dim, output_embed_dim, bias=False) if embed_dim != output_embed_dim and not args.tie_adaptive_weights else None
        else:
...
class LightConvEncoderLayer(nn.Module):
    ...
    def __init__(self, args, kernel_size=0):
        ...
        self.use_dropout_linearity = False
        self.use_snu = False#args.use_snu

        if args.encoder_glu:
            if self.use_dropout_linearity:
                self.linear1 = DropoutLinear(self.embed_dim, 2*self.conv_dim)
            elif self.use_snu:
                self.linear1 = LinearSNU(self.embed_dim, 2*self.conv_dim)
            else:
                self.linear1 = Linear(self.embed_dim, 2*self.conv_dim)
            self.act = nn.GLU()
        else:
            if self.use_dropout_linearity:
                self.linear1 = DropoutLinear(self.embed_dim, self.conv_dim)
            elif self.use_snu:
                self.linear1 = LinearSNU(self.embed_dim, self.conv_dim)
            else:
        ...
        if self.use_dropout_linearity:
            self.linear2 = DropoutLinear(self.conv_dim, self.embed_dim)
        elif self.use_snu:
            self.linear2 = LinearSNU(self.conv_dim, self.embed_dim)
        else:
        ...
        if self.use_dropout_linearity:
            self.fc1 = DropoutLinear(self.embed_dim, args.encoder_ffn_embed_dim)
            self.fc2 = DropoutLinear(args.encoder_ffn_embed_dim, self.embed_dim)
        elif self.use_snu:
            self.fc1 = LinearSNU(self.embed_dim, args.encoder_ffn_embed_dim)
            self.fc2 = LinearSNU(args.encoder_ffn_embed_dim, self.embed_dim)
        else:
...
class LightConvDecoderLayer(nn.Module):
    ...
    def __init__(self, args, no_encoder_attn=False, kernel_size=0):
        ...
        if self.use_dropout_linearity:
            self.fc1 = DropoutLinear(self.embed_dim, args.decoder_ffn_embed_dim)
            self.fc2 = DropoutLinear(args.decoder_ffn_embed_dim, self.embed_dim)
        elif self.use_snu:
            self.fc1 = LinearSNU(self.embed_dim, args.decoder_ffn_embed_dim)
            self.fc2 = LinearSNU(args.decoder_ffn_embed_dim, self.embed_dim)
        else:
...
@register_model_architecture('rnnlightconv', 'rnnlightconv')
...
@register_model_architecture('rnnlightconv', 'rnnlightconv_iwslt_de_en')
def rnnlightconv_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    ...
    args.encoder_conv_type = getattr(args, 'encoder_conv_type', 'lightweight')
    args.decoder_conv_type = getattr(args, 'decoder_conv_type', 'lightweight')
    base_architecture(args)


@register_model_architecture('rnnlightconv', 'rnnlightconv_iwslt_de_en_droplin')
def rnnlightconv_iwslt_de_en_droplin(args):
    args.use_dropout_linearity = getattr(args, 'use_dropout_linearity', True)
    args.use_snu = getattr(args, 'use_snu', False)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 1024)
    args.encoder_attention_heads = getattr(args, 'encoder_attention_heads', 4)
    args.encoder_layers = getattr(args, 'encoder_layers', 7)
...
@register_model_architecture('rnnlightconv', 'rnnlightconv_iwslt_de_en_snu')
def rnnlightconv_iwslt_de_en_droplin(args):
    args.use_dropout_linearity = getattr(args, 'use_dropout_linearity', False)
    args.use_snu = getattr(args, 'use_snu', True)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 512)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 1024)
    args.encoder_attention_heads = getattr(args, 'encoder_attention_heads', 4)
    args.encoder_layers = getattr(args, 'encoder_layers', 7)
...
@register_model_architecture('rnnlightconv', 'rnnlightconv_wmt_en_de')
def rnnlightconv_wmt_en_de(args):
    base_architecture(args)


@register_model_architecture('rnnlightconv', 'rnnlightconv_wmt_en_de_big')
def rnnlightconv_wmt_en_de_big(args):
    args.attention_dropout = getattr(args, 'attention_dropout', 0.1)
...
@register_model_architecture('rnnlightconv', 'rnnlightconv_wmt_en_fr_big')
def rnnlightconv_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    rnnlightconv_wmt_en_de_big(args)


@register_model_architecture('rnnlightconv', 'rnnlightconv_wmt_zh_en_big')
def rnnlightconv_wmt_zh_en_big(args):
