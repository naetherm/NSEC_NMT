...
from fairseq.models import (
    FairseqMultiEncoderSingleDecoderModel,
    register_model,
...
@register_model('rnn_fconv')
class RNNFConvModel(FairseqMultiEncoderSingleDecoderModel):
    """
    ...
    def __init__(self, encoders, decoder):
        super().__init__(encoders, decoder)
    ...
    def build_model(cls, args, task):
        ...
        # TODO(naetherm): Add the RNN based encoder

        fconv_encoder = RNNFConvEncoder(
        ...
        return RNNFConvModel([fconv_encoder], decoder)

class RNNLSTMEncoder(FairseqEncoder):
    """
    Specialized LSTM encoder implementation.
    """
    def __init__(
        self,
        dictionary,
        ...
        # TODO(naetherm): Make this variable so we can easily use e.g. GRU instead of LSTM
...
class RNNFConvEncoder(FairseqEncoder):
    """
    Convolutional encoder consisting of `len(convolutions)` layers.

    Keep in mind that this is a specialized fconv encoder which will not 
    embed the incoming src_tokens any further in the forward method because
    ...
    def __init__(
        ...
        convolutions=((512, 3),) * 20, dropout=0.1, batch_norm=False,
    ):
        super().__init__(dictionary)
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        # TODO(naetherm): This is now deprecated, we will use the original, incoming data
        x = src_tokens['encoder_out'][0] + self.embed_positions(src_tokens)
        ...
        # TODO(naetherm): The masking is directly given in src_tokens
        # used to mask padding in input
        if src_tokens['encoder_padding_mask'] is not None:
          encoder_padding_mask = src_tokens['encoder_padding_mask']
        else:
...
class RNNFConvDecoder(FairseqIncrementalDecoder):
...
@register_model_architecture('rnn_fconv', 'rnn_fconv')
...
@register_model_architecture('rnn_fconv', 'rnn_fconv_iwslt_de_en_light')
def rnn_fconv_iwslt_de_en_light(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 256)
    args.encoder_layers = getattr(args, 'encoder_layers', '[(128, 3)] * 2')
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 256)
    args.decoder_layers = getattr(args, 'decoder_layers', '[(128, 3)] * 2')
    args.decoder_out_embed_dim = getattr(args, 'decoder_out_embed_dim', 256)
    base_architecture(args)

@register_model_architecture('rnn_fconv', 'rnn_fconv_iwslt_de_en_7layer')
def rnn_fconv_iwslt_de_en_7layer(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 256)
    args.encoder_layers = getattr(args, 'encoder_layers', '[(512, 3)] * 7')
    args.decoder_embed_dim = getattr(args, 'decoder_embed_dim', 256)
    args.decoder_layers = getattr(args, 'decoder_layers', '[(512, 3)] * 6')
    args.decoder_out_embed_dim = getattr(args, 'decoder_out_embed_dim', 256)
    base_architecture(args)

@register_model_architecture('rnn_fconv', 'rnn_fconv_iwslt_de_en')
def rnn_fconv_iwslt_de_en(args):
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 256)
...
@register_model_architecture('rnn_fconv', 'rnn_fconv_wmt_en_ro')
def rnn_fconv_wmt_en_ro(args):
    args.decoder_out_embed_dim = getattr(args, 'decoder_out_embed_dim', 512)
    base_architecture(args)


@register_model_architecture('rnn_fconv', 'rnn_fconv_wmt_en_de')
def rnn_fconv_wmt_en_de(args):
...
@register_model_architecture('rnn_fconv', 'rnn_fconv_wmt_en_fr')
def rnn_fconv_wmt_en_fr(args):
