...
"""
This mechanism is highly inspired by AllenNLP.
"""
...
class Field(Generic[DataArray]):
  """
  A :class: ``Field`` is some piece of data that ends up as a tensor in a model.
  Thereby this can either be an input or an output, being quite similar to the 
  placeholder mechanism of tensorflow.

  Fields go through up to two steps of processing:
  (1) Tokenized fields are converted into token ids
  (2) Fields containing token ids (or any other numeric data) are padded (if necessary)
      and converted to tensors. 

  The API has methods around both of these steps, though they may not be needed
  for some concrete inherited classes - if your field doesn't have any strings 
  that need indexing you don't neet to implement ``count_vocab_items`` or ``index``.

  Once a dictionary is computed and all fields are inedexed, we will determine the 
  padding lengths, then inteillgently batch together instances and pad them into 
  actual tensors.
  """
  ...
  def as_tensor(self, padding_length: Dict[str, int]) -> DataArray:
    """
