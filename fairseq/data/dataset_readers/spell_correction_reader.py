...
@DatasetReader.register("spell_correction")
class SpellCorrectionDatasetReader(DatasetReader):
  ...
  def __init__(
    ...
    super(SpellCorrectionDatasetReader, self).__init__(lazy)
  ...
  def _read(self, file_path: str):
    with open(file_path, 'r') as fin:
      ...
      for line_num, row in enumerate(csv.reader(fin, delimiter=self.delimiter, quoting=csv.QUOTE_NONE)):
        if len(row) != 2:
          pass # ERROR
        source_, target_ = row
